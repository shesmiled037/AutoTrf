...
upload_page = "https://kingbokep.fit/page/upload.php"
local_folder = "bokep_pages"
...
while True:
    ...
    if r.status_code != 200:
        print(f"[-] Stop, tidak ada page {page_num}")
        break
    ...
    # ====== Simpan page ======
    subfolder = os.path.join(local_folder, str(page_num))
    os.makedirs(subfolder, exist_ok=True)
    local_file = os.path.join(subfolder, "index.html")
    ...
    with open(local_file, "rb") as f:
        ...
        data = {'folder': str(page_num)}
        resp = requests.post(upload_page, files=files, data=data)
    print(f"    Page upload resp: {resp.text}")
